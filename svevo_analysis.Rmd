...
# remove punctuation and numbers, then remove stop-words
clean_text <- function(corpus) {
  for(i in 1:nrow(corpus)) {
    corpus$text[i] <- gsub("[^[:alpha:][:space:]]*","", corpus$text[i])
    
    if(corpus$languages[i] == "ENG")
      corpus$text[i] <- removeWords(corpus$text[i], stopwords("english"))
    else if (corpus$languages[i] == "ITA")
      corpus$text[i] <- removeWords(corpus$text[i], stopwords("italian"))
    else
      corpus$text[i] <- removeWords(corpus$text[i], stopwords("french"))
  }
