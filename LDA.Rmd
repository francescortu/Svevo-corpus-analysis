...
lda_model_ita$labels <- c("pensieri", "amore", "letteratura", "vizi", "scrivere")
...
sum_topic %>%
    ...
    ggplot() +
    geom_line(aes(x=date, y=value/n, colour=key, group = 1),ylab="")+
    geom_point(aes(x=date, y=value/n, colour=key, group = 1),ylab="")

sum_topic %>%
  gather(key, value, lda_model_ita$labels) %>%
  ggplot( aes(x = date, y = value/n, fill=key, group = 1),ylab="") +
...
sum_topic <- topic_people %>%
  group_by(pair) %>%
  summarise(across(everything(), sum)) %>%
  filter(pair %in% top_receivers$pair) 

sum_topic$n <- for(i in 1:nrow(sum_topic)){return(top_receivers[which(top_receivers$pair == sum_topic$pair[i]),]$n)}

sum_topic <- melt(sum_topic, id = "pair")

sum_topic %>%
  ggplot() + 
  geom_bar(aes(x = pair, y = value/n, fill = variable), position = "dodge", stat = "identity")
