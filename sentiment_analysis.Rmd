...
CLassification using positive and negative files
```{r}
# library(tidyverse)
# library(tidytext)
# library(textdata)
...
get_sentiment <- function(doc) {
  ...
  doc$polarity <- 0
  for(i in 1:length(doc$word)) {
    if(nrow(sentiment[which(sentiment$word == doc$word[i]),])){
      doc$sentiment[i] = sentiment[which(sentiment$word == doc$word[i]),][1,]$sentiment
      doc$polarity[i] = sentiment[which(sentiment$word == doc$word[i]),][1,]$polarity
    }
  }
  return(doc)
}
